...
const SearchBar = ({searchSwitchTodo, todoData}) => {
    ...
    useEffect(() => {
        const initiateSearch = (item) => {
            const filteredResults = todoData.filter(result => (
    ...
    }, [searchSwitchTodo, searchItem, todoData])
    ...
    return (
        <div className='min-w-[40%] lg:min-w-[50%] xl:w-[33rem] 2xl:w-[44rem]'>
            ...
            {showSearchPanel && (
                <div 
                className={`absolute w-[98%] h-[31.5rem] rounded-lg overflow-hidden left-1/2 transform -translate-x-1/2 top-14 ${searchSwitchTodo ? 'bg-slate-900' : ' bg-slate-950'} shadow-2xl shadow-red-300 z-50`}>
                    {searchItemFound && (
                        <div 
                        className={`
                    ...
                    <div className='fixed w-full top-14 h-[28rem] overflow-auto '>
                        {searchItemFound ? (
                            <motion.div 
                            className={` ${searchSwitchTodo ? 'text-cyan-200' : ' text-indigo-300'} space-y-4 px-4 pb-4 `}
                            initial={{y: 300}}
                            animate={{y: 0, duration: 1}}>
                                {searchResult.map((val) => (
                                    <div 
                                    className={`w-full my-[5px] flex flex-col items-start justify-between gap-y-4 tracking-wide hover:ring-[2px] ${searchSwitchTodo ? 'bg-gray-950 hover:ring-emerald-300' : 'bg-gray-900 hover:ring-violet-300'} rounded-lg pl-4 py-2 `}
                                    key={val.id}>
                                        <p className={`${searchSwitchTodo ? 'text-emerald-300' : 'text-violet-300'}  text-[1.4rem] font-montserrat`}>
                                            {val.title}
                                        </p>
                            ...
                            <div className={`absolute top-0 w-full h-full flex flex-col items-center justify-center ${searchSwitchTodo ? 'text-cyan-200' : ' text-amber-300'}`}>
                                <img src={noFound} />
