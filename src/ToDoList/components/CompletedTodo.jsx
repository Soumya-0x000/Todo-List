        ...
        <div className='space-y-2 rounded-lg overflow-hidden'>
            {completedTasks.map((task, index) => (
                <motion.div
                    ...
                    key={index}
                    className='relative hover:ring-[1px] hover:ring-green-300 bg-slate-950 flex justify-between items-center px-3 lg:px-6 py-'>
                    <motion.div
                        initial={{ y: -100 }}
                        animate={{ y: 0 }}
                        transition={{ delay: 0.2, damping: 1 }}
                        className='xl:w-[70rem] 2xl:w-[80rem] py-2 h-auto mr-2 overflow-x-auto scrollbar-thin scrollbar-thumb-lime-300 space-y-2 sm:space-y-3 text-white'>
